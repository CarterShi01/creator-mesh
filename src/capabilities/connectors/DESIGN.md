...
## First Adapter: Notion (design complete)

`src/connectors/notion/` contains the design for the first reference connector adapter.

`NotionConnectorAdapter` implements `ConnectorPort` using the Notion SDK. MVP capabilities: `search/page`, `read/page`, `read/block`, `create/page`, `append/block`. Auth via API key (`NOTION_API_KEY`). Normalizes Notion responses to internal `NotionPageData` / `NotionBlockData` shapes. Pagination handled internally for block reads; cursor-exposed for search. Errors mapped to structured codes. No implementation files yet.

See `src/connectors/notion/DESIGN.md` and `src/connectors/notion/INTERFACE.md` for the full design.
